...
def parse_args():
    ...
    parser.add_option(
        "-m", "--main-instance-type", default="t2.micro",
        help="Main instance type (default: %default).")
    parser.add_option(
...
def setup_cluster(conn, main_nodes, subordinate_nodes, opts, deploy_ssh_key):
    ...
    setup_spark_cluster(main, subordinate_nodes, opts)
    print("Done!")


def setup_spark_cluster(main, subordinate_nodes, opts):
    if opts.gpu:
        scp(main, opts, "%s/gpu/spark_init.sh" % SPARK_EC2_DIR, "spark-ec2/spark/init.sh")
        scp(main, opts, "%s/gpu/spark-defaults.conf.add" % SPARK_EC2_DIR, "spark-ec2/templates/root/spark/conf/spark-defaults.conf.add")
        scp(main, opts, "%s/gpu/spark-env.sh.add" % SPARK_EC2_DIR, "spark-ec2/templates/root/spark/conf/spark-env.sh.add")
        ssh(main, opts, "cat spark-ec2/templates/root/spark/conf/spark-defaults.conf.add >> spark-ec2/templates/root/spark/conf/spark-defaults.conf")
        ssh(main, opts, "cat spark-ec2/templates/root/spark/conf/spark-env.sh.add >> spark-ec2/templates/root/spark/conf/spark-env.sh")
        scp(main, opts, "%s/gpu/create_image.sh" % SPARK_EC2_DIR, "create_image.sh")
        ssh(main, opts, "chmod u+x ./create_image.sh")
        ssh(main, opts, "./create_image.sh > ./create_image_main.log")
        for subordinate in subordinate_nodes:
            subordinate_address = get_dns_name(subordinate, opts.private_ips)
            scp(subordinate_address, opts, "%s/gpu/create_image.sh" % SPARK_EC2_DIR, "create_image.sh")
            ssh(subordinate_address, opts, "chmod u+x ./create_image.sh")
            ssh(subordinate_address, opts, "./create_image.sh gpu > ./create_image_subordinate.log")
